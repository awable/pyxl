...
class PyxlParser(HTMLTokenizer):
    ...
    def handle_starttag(self, tag, attrs, call=True):
        ...
        if tag == 'if':
            ...
            self.output.append('(not html._last_if_condition) and (')
            self.last_thing_was_python = False
            self.last_thing_was_close_if_tag = False
            return
        elif tag == 'for':
            if len(attrs) > 2:
                raise ParseError("for tag takes 2 args at most", self.end)
            if 'var' not in attrs:
                raise ParseError("if tag must contain the 'var' attr", self.end)
            if 'in' not in attrs:
                raise ParseError("if tag must contain the 'in' attr", self.end)
    ...
    def handle_endtag(self, tag_name, call=True):
        ...
        if open_tag['tag'] == 'for':
            ...
            self._handle_attr_value(open_tag['attrs']['var'])
            self.output.append(" in ")
            self._handle_attr_value(open_tag['attrs']['in'])
            self.output.append(")")
